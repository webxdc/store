    ...
    return (
        <form class="flex flex-col gap-2 p-4 rounded shadow max-width bg-white border">
            <label class="flex items-center">
                <input type="checkbox" name='android' checked={props.testStatus.android} onClick={handleInputChange} />
                <span class="ml-2">Works on Android</span>
            </label>
            <label class="flex items-center">
                <input type="checkbox" name='ios' checked={props.testStatus.ios} onClick={handleInputChange} />
                <span class="ml-2">Works on iOS</span>
            </label>
            <label class="flex items-center">
                <input type="checkbox" name='desktop' checked={props.testStatus.desktop} onClick={handleInputChange} />
...
const AppDetail: Component = () => {
    ...
    if (import.meta.env.DEV) {
        setAppInfo({
            ...
            author_name: "xxde@you.de",
            author_email: "uditaenr",
                ...
                <p class="text-gray-500 font-italic">
                    <Show when={!is_complete()} fallback={
                        ...
                        <Show when={!is_appdata_complete()}>
                            <p>Some app data is still missing</p>
                        </Show>
                        <Show when={!is_testing_complete()}>
                            <p>Testing is incomplete</p>
                        </Show>
                    </Show>
                </p>
                <button class="btn font-semibold text-gray-500" classList={{ "bg-gray-100 border-gray-500": !is_complete() }}
